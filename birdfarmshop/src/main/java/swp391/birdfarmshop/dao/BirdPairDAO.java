        ...
        try {
            ...
            if (con != null) {
                String sql = "SELECT COUNT(pair_id) AS [total_birdPair],\n"
                        + "	   COUNT(CASE WHEN [status]= N'Ðã thanh toán' THEN 1 END) AS [payment],\n"
                        + "	   COUNT(CASE WHEN [status]= N'Ch? l?y chim' THEN 1 END) AS [waiting_bird],\n"
                        + "	   COUNT(CASE WHEN [status]= N'Ðang ghép c?p' THEN 1 END) AS [pairing],\n"
                        + "	   COUNT(CASE WHEN [status]= N'Ðã sinh s?n' THEN 1 END) AS [spawned],\n"
                        + "	   COUNT(CASE WHEN [status]= N'Ðã ?p n?' THEN 1 END) AS [hatched]\n"
